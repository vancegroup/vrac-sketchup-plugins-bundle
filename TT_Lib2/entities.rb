...
module TT::Entities
  ...
  #
	# @return [Integer]
	# @since 2.5.0
  def self.count_unique_entities( context )
    c = 0
    entities = nil # Init variables for speed
    self.each_entities( context ) { |entities| c = c.next  }
    c
  end
  ...
  #
	# @return [Integer]
	# @since 2.5.0
  def self.count_unique_entity( context )
    c = context.length
    entities = nil # Init variables for speed
    self.each_entities( context ) { |entities|
      c += entities.length
    }
    c
  end
  ...
  #
	...
	# @since 2.5.0
  def self.each_entities( context, processed_definitions={}, &block )
    c = 0
    ...
    for e in context.to_a
      ...
      if processed_definitions[d].nil?
        ...
        result = self.each_entities( d.entities, processed_definitions, &block )
        c += result if result.is_a?( Numeric )
      end
    end
    c
  ...
  def self.each_entity( entities, processed_definitions={}, &block )
    c = 0
    for e in entities.to_a
      ...
      if processed_definitions[d].nil?
        processed_definitions[d] = true
        c += self.each_entity( d.entities, processed_definitions, &block )
      end
    end # for
    c
  end
