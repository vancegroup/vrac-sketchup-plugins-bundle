...
def G6.oldcurl_follow_extend(edge, anglemax, stop_at_crossing=false, &proc)
	common_normal = nil
	ls_edges = []
	[edge.start, edge.end].each do |vertex|
		edgenext = edge
		while edgenext
			...
			if len == 1
				...
				le.each do |e|
					...
					an = Math::PI - G6.edges_angle_at_vertex(edgenext, e, vertex)
					next if an > anglemax
					puts "an = #{an.radians} max= #{anglemax.radians}"
					if false && common_normal && common_normal.valid?
						vn = G6.edges_normal(e, edgenext)
						puts "VN valid = #{vn.valid?}"
						puts "VN parallel = #{vn.valid? && vn.parallel?(common_normal)} ang = #{vn.angle_between(common_normal).radians}"
...
def G6.pick_component(view, ip, x, y, entity=nil)
	...
	if entity == nil || entity.parent == model || entity.parent == nil
		puts "parent nil = #{entity}" if entity && entity.parent == nil
		parent = model
		tr = @@tr_id
